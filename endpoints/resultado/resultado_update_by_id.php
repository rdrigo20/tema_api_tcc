...
// Função de callback para atualizar um post por id resultado
function update_resultado_by_id(WP_REST_Request $request) {
    ...
    // 3. Valida se a resultado existe
    ...
    if (is_wp_error($resultado)) {
        return new WP_Error('post_update_failed', 'Falha ao atualizar a resultado: ' . $resultado->get_error_message(), array('status' => 500));
    }

    // Se deu sucesso
    return new WP_REST_Response(array('message' => 'resultado atualizada com sucesso', 'id' => $post->ID), 200);
}

// Função para registrar o endpoint
function registrar_update_resultado_by_id() {
    register_rest_route('api', '/resultado/(?P<id>\d+)', array(
        'methods' => 'PUT',
        'callback' => 'update_resultado_by_id',
        // 'permission_callback' => function() { return current_user_can('edit_posts'); } // Quando quiser ligar a segurança, use isto!
    ));
}
add_action('rest_api_init', 'registrar_update_resultado_by_id');
?>